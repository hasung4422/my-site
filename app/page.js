...
export default function HomePage() {
  ...
  const handleLogout = () => {
    setIsLoggedIn(false)
    localStorage.removeItem('adminName') // 로그아웃 시 기록 삭제
  }
      ...
      <div style={{ width: '260px', backgroundColor: '#002b5b', color: 'white', display: 'flex', flexDirection: 'column', padding: '20px' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <Link href="/write" style={{ textDecoration: 'none' }}>
            <button style={{ width: '100%', padding: '15px', textAlign: 'left', backgroundColor: '#003d82', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '10px' }}>📝 작업일보 작성하기</button>
          </Link>
          <Link href="/settings" style={{ textDecoration: 'none' }}>
            <button style={{ width: '100%', padding: '15px', textAlign: 'left', backgroundColor: 'transparent', color: '#ffffffcc', border: 'none', borderRadius: '8px', cursor: 'pointer', marginTop: '10px', display: 'flex', alignItems: 'center', gap: '10px' }}>⚙️ 시스템 설정</button>
          </Link>
          <button style={{ width: '100%', padding: '15px', textAlign: 'left', backgroundColor: 'transparent', color: '#ffffff99', border: 'none', marginTop: '10px', cursor: 'not-allowed' }}>📊 현장 현황 (준비중)</button>
        </div>
